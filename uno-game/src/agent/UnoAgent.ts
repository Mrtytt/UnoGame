...
class UnoAgent {
  ...
  predict(state: number[]) {
    ...
    const predictionArray: number[] = prediction.arraySync(); 
  ...
  async train() {
    ...
    const qValues = await this.model.predict(tf.tensor2d(states)) as tf.Tensor;
    const maxFutureQ = await this.model.predict(tf.tensor2d(nextStates)) as tf.Tensor;

    for (let i = 0; i < batch.length; i++) {
      const action = actions[i];
      const reward = rewards[i];
      const done = doneFlags[i];
      const targetQ = reward + (done ? 0 : this.DISCOUNT_FACTOR * Math.max(...maxFutureQ.arraySync()[i]));
      qValues.arraySync()[i][action] = targetQ;
    }

    // Modeli minibatch ile eğit
    await this.model.fit(tf.tensor2d(states), tf.tensor2d(qValues), { batchSize: this.BATCH_SIZE });
  }
}
